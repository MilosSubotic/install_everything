...
# Settings.
    ...
    #TODO Version compare in some python.
    
    if awk "BEGIN {exit !($VERSION > 20)}"
    then
        FILE_VERSION=`
            echo $VERSION | awk -F. '{
                if(NF == 2){
...
#NOTE: v23 need license for sim.
if awk "BEGIN {exit !($VERSION >= 23)}"
then
    wget https://cdrdv2.intel.com/v1/dl/getContent/825277/825299?filename=qinst-lite-linux-23.1std.1-993.run
    chmod a+x qinst-lite-linux-*.run
    #TODO download with script
    # Do not start install after download
    ./qinst-lite-linux-*.run
fi

###############################################################################
# Check if we have good version of Quartus downloaded.

C=`ls -w1 Quartus*Setup-$FILE_VERSION*-linux.run | wc -l`
if (( $C < 1 ))
then
...
chmod a+x *.run
sudo ./Quartus*Setup-$FILE_VERSION*-linux.run \
    --mode unattended \
